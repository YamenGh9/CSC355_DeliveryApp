...
export default class Customer extends User {
  ...
  static async register(email, password) {
    logger.info("Looking if email exists");
    ...
    logger.info("Registering a Customer");
    ...
    try {
      ...
      logger.error(error.message); // logs DB errors without crashing — newCustomer is still returned below
    }
  ...
  static async login(email, password) {
    ...
    if (!accountExists) {
      logger.info("Email does not have an account");
      throw new Error("Email not found"); // throws so the caller knows login failed
    }
    logger.info("Authenticating a Customer");
